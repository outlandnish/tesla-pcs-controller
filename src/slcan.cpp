...
// Snoop host-originated TX frames for the PCS enable signals.
// HVP_pcsControl (0x22A) signal layout per Model3_ETH.compact.json:
//   bits  0-15: dcLinkVoltageRequest (scale=0.1, signed)
//   bits 16-17: pcsControlRequest
//   bit     18: pcsChargeHwEnabled  -> byte[2] bit 2
//   bit     19: pcsDcdcHwEnabled    -> byte[2] bit 3
...
static void applyEnablesFromTxFrame(const CAN_FRAME& f) {
  ...
  bool chargeOn = (f.data.uint8[2] & 0x04) != 0;
  bool dcdcOn   = (f.data.uint8[2] & 0x08) != 0;
