...
namespace Param
{
   #define PARAM_ENTRY(category, displayName, name, unit, min, max, def, id) name,
   #define TESTP_ENTRY(category, displayName, name, unit, min, max, def, id) name,
   #define VALUE_ENTRY(displayName, name, unit, id) name,
   typedef enum
   {
       PARAM_LIST
       PARAM_LAST,
   ...
   void Init();  // Initialize parameter system (call before FreeRTOS starts)
